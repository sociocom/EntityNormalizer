...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='''Command line tool for normalizing entities based on a dictionary.
...
If the entity does not produce any match in the dictionary, it will be normalized to [NO_MATCH]. If the entity is found in the dictionary but the normalization is empty, it will be normalized to [NO_NORM_FOUND].''',
    ...
    parser.add_argument('dictionary', type=str, help='Normalization dictionary file path')
    ...
    parser.add_argument('--matching_threshold', type=int,
                        ...
                        default=50)
    parser.add_argument('--index', action='store_true', help='Use column indexes instead of names')
    ...
    ## Normalize entities
    normalization_dictionary = EntityDictionary(args.dictionary, args.source, args.target, index=args.index)
    normalized = normalize(text_entities, normalization_dictionary, matching_threshold=args.matching_threshold)
